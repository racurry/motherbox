...
- **native installs**: use a first-party installer when it owns installation and updates without pulling in a managed runtime. Script if possible, manual download if not. Eg, `brew` itself, `claude`, `codex`, `atuin`, `rustup`.
